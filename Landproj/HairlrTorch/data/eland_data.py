...
class ElandDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # imgsize=224
        halfhead=cv2.resize(img,(self.imgsize,self.imgsize))

        # etou_land_croped=etou_land_croped.T

        hairrct=np.array([pt1,pt2],np.float32)*2


        if random.random()<0.5:
            halfhead = cv2.flip(halfhead, 1)
            hairrct[0][0] = 2048 - hairrct[0][0]
            hairrct[1][0] = 2048 - hairrct[1][0]
        ...
        return to_tensor(halfhead),hairrct
...
if __name__ == '__main__':
    ...
    for img, hairrct in traindata_loader:
        print("img shape", img.shape)
        print("landmark size", hairrct.size())
        # img=img.asnumpy()
        ...
        for i in range(0,2):
            cv2.circle(img, (int(hairrct[0][i*2]),int(hairrct[0][i*2+1])), 10, (0, 255, 0), 10)
