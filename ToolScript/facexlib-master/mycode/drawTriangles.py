...
def generateRandomPoints(count, sizeX, sizeY):
    ...
    for i in range(count):
        ...
        if not p in points:
            points.append(p)
    # print "Punkte generieren: %.2fs" % (time.time()-start)
    return points
...
def drawTriangulation(triangles, filename, sizeX, sizeY, multiplier):
    ...
    # im.save(filename, "JPEG")
    # print "Dreiecke zeichnen: %.2fs" % (time.time()-start)
    return np.array(im)
...
def drawImageColoredVoronoi(polygons, filename, origIm, multiplier):
    ...
    ImageFile.MAXBLOCK = im.size[0] * im.size[1]
    im.save(filename, "JPEG", quality=100, optimize=True, progressive=True)
    # print "Voronoi zeichnen: %.2fs" % (time.time()-start)


def generateTriangles(points):
    start = time.time()
    triangles = delaunay(points)
    # print "Delaunay-Triangulierung: %.2fs" % (time.time()-start)
    return triangles
...
def findPointsFromImage(im, factor):
    ...
    # print "Punkte extrahieren: %.2fs" % (time.time()-start)
    return points
...
def loadAndFilterImage(name):
    ...
    im = im.filter(ImageFilter.GaussianBlur(radius=5))
    # print "Bild laden: %.2fs" % (time.time()-start)
    return (orig, im)
...
def autocontrastImage(input_filename, output_filename):
    ...
    # print "Autocontrast Image: %.2fs" % (time.time()-start)
...
def equalizeImage(filename):
    ...
    # print "Equalize Image: %.2fs" % (time.time()-start)
...
def voronoiFromTriangles(triangles):
    ...
    # print "Voronoi-Polygonalisierung: %.2fs" % (time.time()-start)
    return polygons
...
#
#     # Values
#     parser.add_argument('-o', '--output', dest='output_filename', default='1.jpg', help='The filename to write the image to. Supported filetypes are BMP, TGA, PNG, and JPEG')
#     parser.add_argument('-i', '--image-file', dest='input_filename', default='/home/tao/Desktop/Picture1.png', help='An image file to use when calculating triangle colors. Image dimensions will override dimensions set by -x and -y.')
#     parser.add_argument('-f', '--factor', dest='factor', default=2.1, help='Factor definition. Determines the number of generated points (recommended value = 2.1 --> ~3000 points)')
...
#             points = generateRandomPoints(100, width, height)
