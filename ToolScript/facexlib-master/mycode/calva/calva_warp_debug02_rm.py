...
def expand_the_pts(exp_base_pts,exp_pts):
    ...
    for i,ept in enumerate(exp_pts):
        ...
        xpt=pt_trans_one([lenth*1.2,0],trans_param_inv)
...
if __name__=='__main__':
    ...
    for i, im in enumerate(ims):
        ...
        for j,landmarks in enumerate(landmark_list):
            ...
            # 构造背景不动点
            Calva_stable_pts=make_backgroud_stable_pts(Calva_expand_pts_result,Calva_bottom_pts,Calva_base_pts)

            make_big_img([Calva_stable_pts,Calva_expand_pts],calva_croped)
            ...
            draw_pts(calva_mat, list(Calva_expand_pts), 30, (0, 0, 255), 30)
            draw_pts(calva_mat, list(Calva_expand_pts_result), 10, (0,255, 255), 10)


        # cv2.imshow('cat',limit_img_auto(np.concatenate([calva_croped,calva_seg,calva_mat],axis=1)))
        cv2.imwrite('1.jpg',limit_img_auto(np.concatenate([calva_croped,calva_seg,calva_mat],axis=1)))
