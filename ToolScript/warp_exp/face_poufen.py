    ...
    with torch.no_grad():
        for i, im in enumerate(ims):
            ...
            for box in bboxes:
                ...
                # poufenimg=get_poufen_img(np.array(land98_in_crop,np.int))
                ...
                # cv2.imshow('poufenimg2', limit_img_auto(poufenimg))
                ...
                for triind in trires:
                    for k in range(0, 3):
                        cv2.line(facealign, tuple(land98_in_crop_int[triind[k % 3]]), tuple(land98_in_crop_int[triind[(k + 1) % 3]]), (0, 255, 0), 10)
